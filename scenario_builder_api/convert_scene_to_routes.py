...
def convert_scene_to_routes(scene_json_path: str, output_dir: str, ego_num: int = None):
    """
    ...
    for idx, ego_data in enumerate(ego_picked):
        ...
        # Create filename
        filename = f"{town.lower()}_{vehicle_safe}_{idx}.xml"
    ...
    for actor in actors:
        ...
        if category == 'walker':
            ...
            if motion_type == 'static':
                ...
                # Skip moving vehicles - they should be in ego_picked
                print(f"[INFO] Skipping moving vehicle {actor_id} (should be in ego_picked)")
                continue
        else:
